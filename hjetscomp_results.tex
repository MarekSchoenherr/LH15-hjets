...
Moving to jettier `terrain', we discuss a number of $h+2$-jet
...
\MEPSatNLO $h+\text{jets}$ production and fixed-order (plus related)
...
in the high-energy limit. With two or more jets in the final state,
...
\MGaMC ($\sim20\%$), where the latter two obtained from merging ME+PS
at NLO are consistent with the results emerging from the different
...
the very first bin, the \GoSam{}+\Sherpa based calculations show a
rather constant rate increase of about 20\% wrt.~\Powheg pointing to a
$K$-factor size one more or less expects for $h+2$-jet production. For
these predictions, we again find remnants of the Sudakov shoulder
effect appearing around $p_\perp(h)\sim60\gev$ (i.e.~around the
two-jet threshold). The right panel of
Figure~\ref{fig:hjetscomp:results:2obs:hpt_j2pt} displays the various
uncertainty bands. The ones stemming from the NLO-based predictions
are more or less of similar size where the \Herwig uncertainty again
may be somewhat on the small side. In comparison to all these, the
\NNLOPS envelopes are expected to be larger reflecting the loss of one
order in accuracy. A clear difference however is not found indicating
that the \NNLOPS estimates are too optimistic. From this point of
view, the larger difference seen between the two \NNLOPS predictions
is no surprise at all -- rather typical for comparisons at the LO+PS
level, and two fairly different parton showers at work.
...
comparison between the second jet results (of the $N=2$ case) with the
first jet results of the $N=1$ case (depicted in
Figures~\ref{fig:hjetscomp:results:1obs:j1pt} and
...
case. \Todo{no good \& short explanation apart from:}
...
\Todo{LOish?}. In the plot range \Hej's prediction is consistent with
the others due to overlapping uncertainties, but this statement gets
...
variable is the two-jet analog of/to the $p_\perp(hj_1)$ variable for
...
forth jet is described by the parton shower only. The behaviour of
