...
NNLO prediction and the matched result from \Todo{\Resbos} that includes the
...
\Todo{\Resbos} predicts a slightly smaller rate for $y(h)<2$ and a
...
the two analytic resummation approaches, \HqT and \Todo{\Resbos},
...
\begin{table}[t!]
  ...
  \begin{tabular}{c||c|c|c}
    ...
    $13.5^{+2.0}_{-2.1}~\mathrm{pb}$ &
    $16.2^{+3.1}_{-2.8}~\mathrm{pb}$ \\\hline
    NNLO \vphantom{$\int\limits_a^b$} & - & - &
    $16.4^{+0.0}_{-0.9}~\mathrm{pb}$ \\
    \hline
  \end{tabular}
...
resulting in a very flat uncertainty band. The central ratio plot
...
\Sherpa \NNLOPS as well as the STWZ approach and \Resbos; the
uncertainties also are of comparable size. \Todo{except for powheg
excl 0 which is double the others, any ideas?}
...
results for $hj$ and $hjj$ are always somewhat below the respective
\GoSam{}+\Sherpa result though the relatively large Monte Carlo
generation cut of $25\gev$ means that the total rates predicted by
\Loopsim should be interpreted with care.
...
\NNLOPS, NLO ME+PS and \Todo{\Resbos} predictions do not suffer from
...
description is degraded to (N)LO. Here, the inclusion of the first
term of the Sudakov resummation, also included in \Powheg, benefits
the BFGLP $hj$ NNLO calculation. However, it can be noticed (see
fourth ratio plot in Figure~\ref{fig:hjetscomp:results:1obs:hpt}) that
apart from \Powheg, \Todo{\Resbos} and the BFGLP $hj$ NNLO calculation,
all other approaches predict a more steeply falling shoulder resulting
in a significantly lower cross section as $p_\perp(h)\to0$.
...
captured by the \Loopsim approach. \Todo{JW: problematic, if that's
the case why does pure NLO get powheg's tail? Isn't it maybe the
generation cut dependence, check back with figure 5, there is a 20\%
rate diff between pure NLO and Loopsim.}
...
although the
latter is somewhat softer. In the bottom ratio panel, the STWZ and \Resbos 
predictions agree very well with one another, partly due to the coincidence 
of their scale choices in the fixed-order part dominating in this observable. 
Both constitute a resummation improved NLO calculation for this observable, 
agreeing with the NNLO prediction at low $p_\perp$. At the largest $p_\perp$ 
values, where no resummation effects are present, deviations rise up to 30\%
due to their fixed scale choice. Conversely, had a fixed-order 
calculation with a $\tfrac{1}{2}H_T^\prime$ as scale been included in this comparsion 
it would fall close to the multijet merged predictions, showcasing the 
still large scale dependence of the $hj$ NLO calculation.
...
similar in size and do not point to any shape changes
