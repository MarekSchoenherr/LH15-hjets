...
h NNLO instead and explain in text? MS: Fine with
me. After all, this is plain NNLO and Sherpa gives the same as HNNLO
here. It is only faster and easier to use and produce uncertainties.
For transparency, I would call it what it is in the plot, we did it
for every other curve in the plot. Opinions?}
...
normalizations.  Here, the \MGaMC choice of a scale that reduces to
$m_h$, rather than $m_h/2$, is clearly noticeable. The upper edge of
the \MGaMC uncertainty band (equal to a scale that reduces to $m_h/2$)
...
(i.e.~no jets above $30$\gev) Higgs boson transverse momentum
...
with some larger deviations evident at very low $p_\perp$. While the
\Sherpa NNLOPS curve starts to undershoot by almost 20\% already at
$50$\gev, the \Powheg \NNLOPS prediction seems to fall off more
rapidly at high $p_\perp$. In fact \HqT gives the hardest prediction
for the tail of the $p_\perp(h)$ distribution, turning out to be
roughly 20\% above the other predictions, which mostly touch the lower
edge of \HqT's uncertainty band. Note that the \MGaMC central scale
...
$p_\perp$. This is what we expect since this is a strong Sudakov
region where resummation effects play a dominant role.

As shown in Figure~\ref{fig:higgscomp:results:exclobs:hpt}, the
exclusive version of $p_\perp(h)$ exhibits deviations among the
predictions that become more sizable, in particular above $30$\gev.
Note that the comparison is now taken wrt.~the \Powheg curve that
agrees with \Sherpa \NNLOPS within the 20\% uncertainty bands.
Similarly, \Sherpa remains inside the \MGaMC envelope, however the
deviations to \Powheg can now become larger than 20\%, or for the case
of \Herwig, they even grow larger than 40\%. In the exclusive zero-jet
case, the $p_\perp(h)$ distribution declines much faster, easily
spanning three orders of magnitude between zero and $100$\gev. Thus,
this is a stringent test of all ME-plus-PS combining predictions, as
...
and soft gluon radiation. From the resummation (i.e.~parton-shower)
...
\Todo{can we explain why HqT is the hardest, some scale choice thingy,
  or just the fact that there's no notion of core scale plus dynamical
  scale setting? Why is HqT's band so large, or is it compared to the
  others that the others underestimate resummation uncertainties? Can
  we say something?}

\Todo{Shall we keep h NNLO in the exclusive plot?} 

\Todo{Shall/Can we comment on Herwig's small variation in exclusive
  case? It's too small, probably underestimated?}
...
We want to obtain a first impression of how the different predictions
...
\Todo{Can't believe the BFGLP NNLO for h+1 gives the correct result,
