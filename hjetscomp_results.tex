...
\begin{table}
  \centering
  \begin{tabular}{cccc}
    order & $\mu_R=m_H/2$ & $\mu_R=\hat{H}_T'/2$ & $\mu_R=\sqrt{\Sigma_T}/2$ \\
    \hline \\
    NLO   & $17.0^{+3.0}_{-2.9}$ pb & $13.5^{+2.0}_{-2.1}$ pb & $16.2^{+3.1}_{-2.8}$ pb \\
    NNLO  & -     & -     & $16.4^{+0.0}_{-0.9}$ pb \\
    \hline
  \end{tabular}
  \caption{The total cross-section for inclusive production of a Higgs boson and one additional jet using different core scale choices.
  The two dynamical scales are $\hat{H}_T'=m_{H,T} + \sum_{\rm partons} p_T$ (see also eq. \eqref{eq:hthatprime}) and $\Sigma_T = m_H^2 + \sum_{\rm jets} p_T^2$ (see also eq. \eqref{eq:bfglpScale}).}
  \label{tab:H1jXS}
\end{table}
