...
the same level. \Todo{the latter was seen better in the old plot
  range; shall we go to higher pt in this type of observable like
  200 or 250?} Recall that for \Sherpa \NNLOPS a third, fourth and so
...
generates harder transverse momentum spectra in general.
\Todo{what's the matter with Hej below 30 Gev?}
...
\Todo{is xsec the same for pt and y ordered correlation? If not shall
  we comment?}
...
shower in the scale setting procedure. \Todo{Can we say the latter
...
$\Delta y=8$, \Todo{betraying its differing
theoretical input. -- sounds harsh ...? In any case JW is puzzled by
Hej's prediction, shouldn't they predict wider separations?}
...
are impressed upon it \Todo{molded in} by the VBF event selection.
\Todo{JW tough sentence, changed somewhat to:
...
mostly adopt small-sized to medium-sized transverse momenta.
 \Todo{JW: dont think that's right: The Higgs boson cannot possess 
large transverse momenta in these situations.}
...
normalization. \Todo{next sentence moved from bottom to here:} \Hej
...
increases to 30\% as $\Delta\phi\to0$. \Todo{Jw; would drop this as
  it's not true from relative point of view: The largest deviation among 
the different calculations is again presented by \Hej.}
