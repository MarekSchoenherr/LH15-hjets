...
\Todo{Do we say in the technicalities section how error bands are
  ...
  describing the plots? MS: Leave it in technicalities where it already 
  is, otherwise it needs to be repetitive.}
...
and soft gluon radiation. Note that the comparison is now taken wrt.\ 
...
\Todo{can we explain why HqT is the hardest, some scale choice thingy,
  ...
  we say something? MS: The high tail in \HqT comes from the fixed 
  scale choice. The band has the same size as for the \NNLOPS ones. 
  The \Powheg one in contrast to \HqT and \Sherpa does not contain 
  the resummation uncertainty as the public code does not offer this 
  option.}
