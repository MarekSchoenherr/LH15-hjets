...
$hjj$ production at ${\cal O}(\alpha_\mathrm{s}^5)$. It is important
...
in the high-energy limit, starting from a LO accurate $h+2j$ configuration. 
...
transverse momenta. Examining first the region with $p_\perp\gtrsim 
60\,\gev$ good agreement between all multijet merged calculations and 
\Powheg is found. Only \Herwig predicts a slightly more quickly falling 
distribution. \Sherpa \NNLOPS, due to its scale choice, exhibits a 
more or less constant offset of $+20\%$ with respect to the \Powheg 
\NNLOPS, but well within their respective uncertainties. In comparison 
to all the NLO uncertainty bands, the \NNLOPS envelopes are expected to 
be larger reflecting the loss of one order in accuracy. A clear 
difference however is not found indicating that the \NNLOPS estimates 
are too optimistic. From this point of view, the larger difference seen 
between the two \NNLOPS predictions is no surprise at all -- rather 
typical for comparisons at the LO+PS level, and two fairly different 
parton showers at work. The fixed-order 
predictions (NLO, \Minlo, \Loopsim) agree well in this region with one 
another, but surpass the \Powheg reference and, more importantly, 
the multijet merged calculations with the same accuracy, by about 
$20\%$. This deviation is just covered by the edge of the NLO uncertainty 
band of either NLO calculation. Finally, \Hej clearly produces the 
hardest spectrum in this region, presenting a clear slope 
with respect to all other calculations, albeit starting from a 
approximately $40\%$ lower cross section at $p_\perp\approx 60\,\gev$.

In the region below the peak, $p_\perp\lesssim 60\,\gev$, the various 
calculations spread widely. Assuming the jets to be produced near 
threshold, then while for $p_\perp(h)>30\,\gev$ the leading and the 
subleading jet have to be in the same hemissphere opposing the Higgs 
boson, for $p_\perp<30\,\gev$ the subleading jet has to cross into the 
Higgs boson's hemisphere opposing the leading jet. This latter region 
receives large resummation effects as both jets recoil mainly against 
each other and soft radiation off them has a large influence on the 
small Higgs boson transverse momentum. Another important role in this 
region is played by the assignment of scales in the context of multijet 
merging. The clustering algorithm needed to define the local CKKW or 
\Minlo scales must allow for the possibility of Higgs radiation of a 
dijet process to assign a meaningful history in this regime if the jets 
are sufficiently hard. In consequence, the value of the scales increases 
and the cross section is reduced, explaining the identical behaviour of 
both the \Sherpa MEPSatNLO and \Sherpa \Minlo predictions. The \Herwig 
and \MGaMC predictions are largely parallel here, offset by the different 
scale choice in \MGaMC. \Powheg \NNLOPS gives the slowest decline of the 
differential cross section as $p_\perp\to 0$. The various fixed-order 
predictions spread somewhat more widely then for large $p_\perp$ and 
start to exhibit a slope with respect to the \Powheg reference, but 
retain a more or less constant ratio with respect to \Herwig or \MGaMC 
evidencing the effect of a scale choice unable to identify the Higgs 
strahlungs component here.
...
\begin{figure}[t!]
  ...
  \caption{\label{fig:hjetscomp:results:2obs:hpt_excl}%
    ...
    supplemented by two ratio plots using the reference result as
    obtained from \Powheg's \NNLOPS calculation for $h$ production.
    The predictions are grouped -- from top to bottom -- according to
    the categories \NNLOPS $h$ production and ME+PS merging at NLO (at
    least up to two jets). \Hej's prediction is added to the first, 
    the \NNLOPS, subpanel.}
\end{figure}
...
\begin{figure}[t!]
  ...
  \caption{\label{fig:hjetscomp:results:2obs:jet2_pt}%
    ...
    \Todo{the HEJ legend fell off the plot.}}
\end{figure}
...
The subleading jet $p_\perp$ spectra for $h+\ge2$ and $h+{}$ exactly
...
Figure \ref{fig:hjetscomp:results:2obs:j2y} depicts the subleading jet's 
...
the procedure's cut-off dependence.

\Todo{the Minlo uncertainty band is still subject to discussion and
  testing}
...
gosam results for $h\,+\!\ge\!\!2$ jets. Similar conclusions hold for for the
...
\begin{figure}[t!]
  ...
  \caption{\label{fig:hjetscomp:results:2obs:dyjj_fb}%
    ...
    \Todo{MS: Updated. Still, do we want it?}}
\end{figure}
