...
As a result of ..., \Todo{\Resbos} predicts a slightly smaller rate,
in particular for $y(h)<2$, than the one given by the NNLO calculation.
The \MGaMC, \Sherpa \MEPSatNLO and \Herwig have slightly lower (NLO)
...
the finite process-dependent pieces in the CFG and CSS schemes. The
\Resbos scale variation band features a cross-over point at
$p_\perp(h)\approx20\gev$ but this does not indicate/imply a vanishing
...
first of all, the agreement between all results is
...
jet bins. Although not shown here, when increasing the minimum
jet-$p_\perp$ threshold to $50\gev$ the picture does not change
significantly. Again, the bottom panels are split up into several ratio
